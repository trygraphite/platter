...
export function MenuCard({ item }: MenuCardProps): JSX.Element {
  ...
  const getDisplayPrice = () => {
    if (item.varieties && item.varieties.length > 0) {
      ...
      return defaultVariety
        ...
        : (item.varieties[0]?.price ?? item.price);
    }
    return item.price;
  };
  ...
  const displayPrice = hasMultipleVarieties
    ? getPriceRange()
  ...
  return (
    <div className="group relative bg-white rounded-lg overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 flex flex-col h-full transform hover:-translate-y-1">
      ...
      <div
        className={cn(
          "flex-1 p-4",
          !item.image && "border-t-4 border-primary/20",
        )}
      >
          ...
          <div className="mt-3 border-t pt-3">
            ...
            <div
              className={cn(
                ...
                hasMultipleVarieties && !showVarieties
                  ? "max-h-0 opacity-0"
                  : "max-h-96 opacity-100",
              )}
            >
              {item.varieties?.map((variety, _index) => (
                <div
                  key={variety.id}
                  className={cn(
