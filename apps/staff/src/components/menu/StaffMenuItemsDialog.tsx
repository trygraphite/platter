...
import { StaffAddMenuItemModal } from "./StaffAddMenuItemModal";
...
interface StaffMenuItemsDialogProps {
  isOpen: boolean;
  onClose: () => void;
  category: Category & { menuItems: MenuItemWithVarieties[] };
}

export function StaffMenuItemsDialog({
  isOpen,
  onClose,
  category,
}: StaffMenuItemsDialogProps) {
  const [isAddModalOpen, setIsAddModalOpen] = useState(false);
  const [editingMenuItem, setEditingMenuItem] =
    useState<MenuItemWithVarieties | null>(null);
  const [expandedItems, setExpandedItems] = useState<Set<string>>(new Set());

  const { editMode, handleDeleteMenuItem, handleUpdateMenuItem } =
    useStaffMenu();
  ...
  const getDisplayPrice = (item: MenuItemWithVarieties) => {
    ...
    const prices =
      item.varieties?.map((v) => v.price).sort((a, b) => a - b) || [];
    if (prices.length === 0) {
                      ...
                      <div className="flex flex-col md:flex-row md:items-center gap-4">
                        ...
                        <div className="flex-grow">
                          <div className="flex items-start justify-between">
                            <div>
                              <h3
                                className={`font-semibold text-lg ${!item.isAvailable ? "text-gray-500" : ""}`}
                              >
                                {item.name}
                                {item.servicePoint && (
                              ...
                              <p
                                className={`text-sm ${!item.isAvailable ? "text-gray-400" : "text-gray-600"}`}
                              >
                              ...
                              {hasVarieties && (
                                <div className="flex items-center gap-2 mt-2">
                                  <Badge variant="secondary">
                                    {item.varieties?.length || 0} varieties
                                  </Badge>
                                  <Button
                                    variant="ghost"
                                    size="sm"
                            ...
                            <div className="flex flex-col items-end">
                              <span
                                className={`font-bold text-lg ${!item.isAvailable ? "text-gray-500" : ""}`}
                              >
                                ...
                                <div className="flex items-start justify-between">
                                  <div>
                                    <div className="flex items-center gap-2">
                                      <span
                                        className={`font-medium ${!variety.isAvailable ? "text-gray-500" : ""}`}
                                      >
                                        {variety.name}
                                      </span>
                                      {variety.isDefault && (
                                    ...
                                    {variety.description && (
                                      <p
                                        className={`text-sm mt-1 ${!variety.isAvailable ? "text-gray-400" : "text-gray-600"}`}
                                      >
                                        {variety.description}
                                      </p>
                                    )}
                                  ...
                                  <span
                                    className={`font-semibold ${!variety.isAvailable ? "text-gray-500" : "text-green-600"}`}
                                  >
      ...
      {editingMenuItem && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg">
            ...
            <Button onClick={() => setEditingMenuItem(null)}>Close</Button>
          </div>
        </div>
      )}
    </>
