...
const orderController = {
  createOrder: async (req: Request, res: Response): Promise<void> => {
    try {
      console.log("Received order creation request body:", JSON.stringify(req.body, null, 2))
      ...
      if (order) {
        ...
        if (order.items && Array.isArray(order.items)) {
          ...
          console.warn("Order is missing items array or it's not properly formatted")
        }

        // Emit socket event for real-time update
        socketManager.emitNewOrder(order)

        res.status(200).json({ success: true, message: "Order notification processed" })
        return
      }
  ...
  getOrder: async (req: Request<{ id: string }>, res: Response): Promise<void> => {
    try {
      const { id } = req.params
  ...
  // Update an order
  updateOrder: async (req: Request<{ id: string }, any, Partial<Order>>, res: Response): Promise<void> => {
    try {
      const { id } = req.params
      const orderData = {
