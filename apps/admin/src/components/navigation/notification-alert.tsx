...
export function NotificationCenter({
  ...
  const [activeWaiterCalls, setActiveWaiterCalls] = useState<
    { tableNumber: string | number; message: string }[]
  >([]);
  ...
  useEffect(() => {
    ...
    if (userId) {
      ...
      console.log(
        `NotificationCenter: Joined restaurant room: ${roomName} for user: ${userId}`,
      );
    }
    ...
    socket.on("waiterAlert", (data) => {
      ...
      setTimeout(() => {
        setActiveWaiterCalls((prev) =>
          prev.filter(
            (call) =>
              !(call.tableNumber === tableNumber && call.message === message),
          ),
        );
      }, 60000);
    ...
    socket.on("notify/call-waiter", (data) => {
      ...
      // Show the special persistent waiter alert toast (60 seconds)
      const _toastId = showWaiterAlertToast(tableNumber, message);
        ...
        <div className="relative">
          ...
          >
            <span className="mr-1 font-medium">
              Table {activeWaiterCalls[0]?.tableNumber}
            </span>
            <Badge variant="outline" className="bg-amber-500">
              {activeWaiterCalls.length > 1
                ? `+${activeWaiterCalls.length - 1}`
                : ""}
            </Badge>
          </Button>
                ...
                {activeWaiterCalls.map((call, index) => (
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        acknowledgeWaiterCall(call.tableNumber, call.message)
                      }
                    >
                      Mark As Read
                    </Button>
