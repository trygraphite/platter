...
export function RevenueChart({ orders, className }: RevenueChartProps) {
  ...
  const years = useMemo(() => {
    ...
    const yearsSet = new Set(orders.map(order => 
  ...
  const data = useMemo(() => {
    ...
    const filteredOrders = timeFrame === 'yearly' 
      ? orders.filter(order => new Date(order.createdAt).getFullYear().toString() === selectedYear)
      : orders;
    ...
    for (const order of filteredOrders) {
      ...
      if (timeFrame === 'last7days') {
        ...
        const daysAgo = Math.floor((currentDate.getTime() - orderDate.getTime()) / (24 * 60 * 60 * 1000));
        ...
        if (daysAgo >= 0 && daysAgo < 7) {
          ...
          revenueByPeriod[periodKey] = (revenueByPeriod[periodKey] || 0) + order.totalAmount;
        }
        continue;
  ...
  const comparisonStat = useMemo(() => {
    ...
    if (timeFrame === 'last7days') {
      ...
      previousPeriodTotal = orders
        .filter(order => new Date(order.createdAt).getFullYear() === currentYear - 1)
        .reduce((sum, order) => sum + order.totalAmount, 0);
    }
    
    // Calculate percentage change
    const percentage = previousPeriodTotal === 0 
    ...
    <Card className={className}>
      ...
      <CardContent className="pb-4">
        ...
        <div className="mt-4 flex items-center">
          ...
          <p className={comparisonStat.isIncrease ? "text-green-500" : "text-red-500"}>
            {comparisonStat.isIncrease ? "+" : "-"}{comparisonStat.percentage}% {" "}
            {timeFrame === 'last7days' ? 'compared to previous 7 days' : 
             timeFrame === 'monthly' ? 'compared to previous month' : 
