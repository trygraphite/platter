...
export function processFeedbackData(
  ...
  const totalComplaints = result.reduce(
    (sum, item) => sum + item.complaints,
    0,
  );

  const reviewsPercentage =
    totalReviews === 0 && totalComplaints === 0
      ? 0
...
function countItemsByDate(
  ...
  dateMap: Map<
    string,
    { date: string; reviews: number; complaints: number; [key: string]: any }
  >,
  ...
  items.forEach((item) => {
    ...
    if (timeFilter === "daily") {
      ...
      for (const [_key, value] of dateMap.entries()) {
