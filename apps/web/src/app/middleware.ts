...
function isSuspiciousRequest(request: NextRequest, ip: string): boolean {
  ...
  if (hashCount > 8) {
    // Same request more than 8 times rapidly
    return true;
  }
  ...
  try {
    ...
    if (!checkWindowLimit(clientIP)) {
      logActivity(
        "BLOCK",
        clientIP,
        "Rate limit exceeded (per window)",
        pathname,
      );
    ...
    response.headers.set(
      "X-RateLimit-Reset",
      Math.ceil((windowStart + config.windowMs) / 1000).toString(),
    );
    ...
    logActivity(
      "INFO",
      clientIP,
      "Middleware error - allowing request",
